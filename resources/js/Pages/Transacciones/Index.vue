<template>
  ...
  <AdminLayout>
    <v-container fluid class="pa-6">
      <div class="mb-5">
        <h5 class="text-h5 font-weight-bold">Gestión de Transacciones</h5>
        <p class="text-medium-emphasis">Visualiza y gestiona las transacciones de ventas</p>
      </div>
      ...
      <v-card class="mb-4">
        <v-card-text class="pa-4">
          <v-row dense>
            ...
            <v-col cols="12" md="2">
              <v-btn color="black" variant="flat" class="text-white" block @click="loadTransactions" :loading="loading">
                Filtrar
              </v-btn>
            </v-col>
          </v-row>
          ...
          >
            ...
            <template v-slot:no-data>
              <div class="text-center pa-4">
                ...
                <p class="text-h6 text-medium-emphasis mt-2">No hay transacciones</p>
              </div>
            </template>
          </v-data-table>
        </v-card-text>
      ...
      <v-card>
        ...
        <v-card-text class="pt-4">
          ...
          <v-alert color="warning" variant="tonal" class="mt-3">
            <div class="text-caption">
              <strong>Nota:</strong> La transacción se mantendrá en los registros pero será marcada como anulada.
            </div>
          </v-alert>
          <div v-if="selectedTransaction" class="mt-4">
            <div><strong>ID:</strong> {{ selectedTransaction.id }}</div>
            <div><strong>Usuario:</strong> {{ selectedTransaction.user_name }}</div>
...
const headers = [
  ...
  { title: 'Usuario', key: 'user_name', sortable: true },
...
const loadTransactions = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/transacciones/list', { params })
...
const viewDetails = async (transaction) => {
  try {
    const response = await axios.get(`/api/transacciones/${transaction.id}/details`)
...
const cancelTransaction = async () => {
  ...
  try {
    await axios.put(`/api/transacciones/${selectedTransaction.value.id}/cancel`, {
      status_id: 2
    })
