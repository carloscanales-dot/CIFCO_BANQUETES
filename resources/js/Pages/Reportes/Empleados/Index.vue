<template>
  <Head title="Reportes de Empleados" />
  <AdminLayout>
    <v-container fluid class="pa-6">
      <div class="mb-5">
        <h5 class="text-h5 font-weight-bold">Reportes de Créditos a Empleados</h5>
        <p class="text-medium-emphasis">Genera y descarga reportes personalizados de créditos otorgados a empleados</p>
      </div>
      ...
      <v-card class="mb-4">
        ...
        <v-card-text class="pa-4">
          ...
          <v-row dense class="mt-2">
            <v-col cols="12">
              <v-btn color="warning" variant="flat" class="text-white" @click="loadReport" :loading="loading" prepend-icon="mdi-magnify">
                Generar Reporte
              </v-btn>
      ...
      <v-row dense class="mb-4" v-if="reportData">
        <v-col cols="12" md="3">
          <v-card variant="outlined" color="warning">
            <v-card-text>
              ...
              <div class="text-caption text-medium-emphasis">Total en Créditos</div>
            </v-card-text>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="3">
          <v-card variant="outlined">
            <v-card-text>
              <div class="text-h5 font-weight-bold">
                {{ reportData.empleados_unicos || 0 }}
              </div>
              <div class="text-caption text-medium-emphasis">Empleados Únicos</div>
            </v-card-text>
          </v-card>
        </v-col>
      ...
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center text-h6 font-weight-bold py-3">
          <span>
            ...
            Detalle de Transacciones a Empleados
          </span>
...
const headers = [
  ...
  { title: 'Empleado', key: 'employee', sortable: true },
...
const loadReport = async () => {
  loading.value = true
  try {
    const response = await axios.get('/api/reportes/empleados/data', { params: filters })
...
const downloadPDF = async () => {
  ...
  try {
    const response = await axios.get('/api/reportes/empleados/download/pdf', {
      params: filters,
      responseType: 'blob'
    })
    ...
    link.href = url
    link.setAttribute('download', `reporte_empleados_${new Date().getTime()}.pdf`)
...
const downloadExcel = async () => {
  ...
  try {
    const response = await axios.get('/api/reportes/empleados/download/excel', {
      params: filters,
      responseType: 'blob'
    })
    ...
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', `reporte_empleados_${new Date().getTime()}.xlsx`)
    document.body.appendChild(link)
    link.click()
    link.remove()
