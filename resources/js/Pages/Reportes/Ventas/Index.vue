          ...
          <v-card>
            ...
            <v-card-text>
              <v-table density="compact">
                ...
                <tbody>
                  <tr v-for="(producto, index) in reportData.productos_mas_vendidos" :key="index">
                    <td>
                      <v-chip size="x-small" :color="index === 0 ? 'success' : 'grey-lighten-1'" class="mr-2">
                        {{ index + 1 }}
                      </v-chip>
                      {{ producto.product_name }}
                    </td>
                    <td class="text-right font-weight-bold">{{ producto.cantidad_vendida }}</td>
                    <td class="text-right">${{ parseFloat(producto.total_vendido).toFixed(2) }}</td>
                  </tr>
                  <tr v-if="!reportData.productos_mas_vendidos || reportData.productos_mas_vendidos.length === 0">
                    <td colspan="3" class="text-center text-medium-emphasis">No hay datos</td>
                  </tr>
                </tbody>
              </v-table>
            </v-card-text>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-table density="compact">
                ...
                <tbody>
                  <tr v-for="(metodo, index) in reportData.totales_por_metodo_pago" :key="index">
                    <td>{{ metodo.payment_method || 'N/A' }}</td>
                    <td class="text-right">{{ metodo.cantidad_transacciones }}</td>
                    <td class="text-right font-weight-bold">${{ parseFloat(metodo.total).toFixed(2) }}</td>
                  </tr>
                  <tr v-if="!reportData.totales_por_metodo_pago || reportData.totales_por_metodo_pago.length === 0">
                    <td colspan="3" class="text-center text-medium-emphasis">No hay datos</td>
                  </tr>
                </tbody>
              </v-table>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-table density="compact">
                ...
                <tbody>
                  <tr v-for="(estacion, index) in reportData.ventas_por_estacion" :key="index">
                    <td>{{ estacion.station_name || 'N/A' }}</td>
                    <td class="text-right">{{ estacion.cantidad_transacciones }}</td>
                    <td class="text-right font-weight-bold">${{ parseFloat(estacion.total).toFixed(2) }}</td>
                    <td class="text-right">{{ ((estacion.total / reportData.total_monto) * 100).toFixed(1) }}%</td>
                  </tr>
                  <tr v-if="!reportData.ventas_por_estacion || reportData.ventas_por_estacion.length === 0">
                    <td colspan="4" class="text-center text-medium-emphasis">No hay datos</td>
                  </tr>
                </tbody>
              </v-table>
            </v-card-text>
...
const loadReport = async () => {
  loading.value = true
  try {
    const response = await axios.get('/api/reportes/data', {
      params: filters,
      timeout: 120000 // 2 minutos de timeout para reportes grandes
    })
    ...
    if (error.code === 'ECONNABORTED') {
      showSnackbar('El reporte está tardando demasiado. Intenta reducir el rango de fechas.', 'warning')
    } else {
...
const downloadPDF = async () => {
  ...
  try {
    const response = await axios.get('/api/reportes/download/pdf', {
      params: filters,
      responseType: 'blob',
      timeout: 180000 // 3 minutos para generación de PDF
    })
    ...
    if (error.code === 'ECONNABORTED') {
      showSnackbar('La generación del PDF está tardando demasiado. Intenta reducir el rango de fechas.', 'warning')
    } else {
...
const downloadExcel = async () => {
  ...
  try {
    const response = await axios.get('/api/reportes/download/excel', {
      params: filters,
      responseType: 'blob',
      timeout: 180000 // 3 minutos para generación de Excel
    })
    ...
    if (error.code === 'ECONNABORTED') {
      showSnackbar('La generación del Excel está tardando demasiado. Intenta reducir el rango de fechas.', 'warning')
    } else {
