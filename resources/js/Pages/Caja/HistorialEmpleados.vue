<template>
  <Head title="Historial de Créditos Empleados" />
  <CreditoLayout>
    <v-container>
      <v-card>
        <v-card-title>Historial de Créditos Empleados</v-card-title>
        ...
        <v-card-text>
          ...
          <div v-else class="text-center pa-6">
            ...
            <div v-else>No hay transacciones de empleados para mostrar.</div>
          </div>
    ...
    <v-dialog v-model="showReprintDialog" max-width="500" persistent>
      <v-card>
        ...
        <v-card-text>
          <p class="mb-2">¿Desea reimprimir el ticket de crédito empleado #{{ selectedTransaction?.id }}?</p>
          <v-alert
            type="info"
            variant="tonal"
            density="compact"
...
const onPageChange = (newPage) => {
  ...
  loading.value = true
  page.value = newPage
  router.get('/historial-empleados', { page: newPage }, {
    preserveState: false,
    onFinish: () => {
      loading.value = false
    }
...
const confirmReprint = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      showToast('Ticket de empleado enviado a impresión correctamente', 'success')
      closeReprintDialog()
    } else {
